...
for i in range(INITIAL_NUM_KEYS):
    user_info = UserInfo(InfoGenerator.generate_user_id(), UserData(InfoGenerator.generate_email(RANDOM_STRING_LENGTH), InfoGenerator.generate_password(PASSWORD_LENGTH)))
...
print ('List node names and key counts:')
...
for i in range(10):
    ...
    print(user_id, first_node.get_data(user_id))
print('\n\n')
...
# Reads the Virtual Node Map stored on external file.
print("\n\n")
option = input("Would you like to read the external file 'vnode_map.txt'? Yes/No: ")
print("\n")
if (option == 'Yes') or  (option == 'yes'):
    print("Reading external text file 'vnode_map.txt'")
    print("\n")
    with open('vnode_map.txt') as file:
        df = file.read()
    print(df)
    print("Thank you!")
else:
    print("You decided to open and review the 'vnode_map.txt' later")
    print("It's available with the source files")
    print("Thank you!")
    print("\n\n")